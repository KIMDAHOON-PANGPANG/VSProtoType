...
| `build/index_23.html` | v0.17.0 — Wave Director(덱 020 #9~20): 15분 스테이지 · 분 단위 웨이브 버짓 밴드(phase1~3 대체) · HP 스케일 커브 · 강화 웨이브 · 이벤트 웨이브 4종(가두리·보석엘리트화·해파리·저지선) · 엘리트 물결형+위압 · 보스 기둥 4개(1차 스코프) |
| `build/index_24.html` | v0.18.0 — Midboss & Punish Rewards(피드백 6건): 차지 무기 방향 고정(불사조) · 카드 설명 3단계 간소화 · 5분 중간보스(광역 리프·처형 2회) · 2분 대량 웨이브+HP 상향+템포 커브(1~2분 카이팅) · 엘리트 3종 로테이션(청소기·격노) · 처형 강화 보상 오브젝트(공용 상시/전용 3회) |
| `build/index_25.html` | v0.19.0 — Rush Waves & Spreader(피드백 6건): 강화 웨이브=고속 러시 변종(×1.8) · 리프 88→120 · 중간보스 리프 240(저스트 회피 강제)+쿨 4.5~6.2 · 전염 엘리트(첫 그로기 미처형 시 확률 변이 — 로스터 4종) · 그로기 2.4s · 보상 픽업 개선(근처 스폰·화살표·반경·수명) |
| `build/index_26.html` | v0.20.0 — Repel & Two-Phase Rewards: 전용 보상 = 2단계 선택(전용 3종 중 1개 → 공용 중 1개) · 중간보스 떨쳐내기(처형 버팀 기상 시 확장 파동 넉백, 마지막 처형 땐 없음) · 중간보스 HP 1200(+50%) |
| `build/index_27.html` | v0.21.0 — Rhythm & Bounce (FQA S001 판정 반영): 초반 감속 철회+후반 가속(템포) · 강화 웨이브 위협화(물량·burst·러시 2.0·접촉딜↑) · 엘리트 공급 에스컬레이션(eliteMax 5·물결 2~4) · 저스트회피→처형 찬스(쿨 환급+즉시 그로기) · 엘리트 방치 비용↑ · 도끼 은퇴 → 신무기 '튕기는 축구공'(ball) · 육각 오라 버프 |
| `build/index_28.html` | v0.22.0 — Escort & Festival (덱 022 이행): 엘리트 호위 편성(자코 12~16 동반·그로기 시체 곁 유지 → 인파이팅 처형) · 엘리트 솔로 삭제(HP ×0.6 분할·축제 슬롯 2~3 동시) · 평시 eliteMax 1 + 전 강화 슬롯 eliteN(농사/축제 리듬) · 공용 처형 카드 개편(환급/쿨 삭제 → 파동·빙결·흡수) (**현재 최신**) |
