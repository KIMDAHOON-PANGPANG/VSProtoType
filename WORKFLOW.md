...
| `build/index_20.html` | v0.15.0 — 레벨업 카드 테두리 공용 teal 통일(진화 임박만 골드) · 무덤 거미 스폰을 고정 배치→PC 앞 성장 연출+랜덤 방향 크롤로 개편 · 부채꼴 베기가 정식 무기(slash Lv1~8+진화)로 편입 · 마우스 에임 폐지 → WASD 이동 방향 360도 회전 (**현재 최신**) |
